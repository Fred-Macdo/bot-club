...
class DataManager:
    ...
    def __init__(self, keys: Dict[str, str], provider_name: str = "yahoo"):
        self.keys = keys
    ...
    def _initialize_provider(self, provider_name: str):
        ...
        try:
            if provider_name == "yahoo":
                ...
                api_key = self.keys.get("alpaca_paper_api_key") or self.keys.get(
                    "alpaca_live_api_key"
                )
                secret_key = self.keys.get("alpaca_paper_secret_key") or self.keys.get(
    ...
    async def fetch_historical_data(
        ...
        if not self.data_provider:
            raise ValueError(
                "Data provider not initialized. Call initialize_provider() first."
            )
        symbols = (
            [symbol + "/USD" for symbol in symbols if symbol in AVAILABLE_CRYPTO_ASSETS]
        ...
        cache_key = (
            f"{'-'.join(symbols)}_{start_date}_{end_date}_{timeframe}_{provider_name}"
        )
        ...
        logger.info(
            f"Fetching data for {symbols} from {start_date} to {end_date} using {provider_name}"
        )

        try:
            start_dt = (
                self._convert_to_datetime(start_date)
                if isinstance(start_date, str)
                else start_date
            )
            end_dt = (
                self._convert_to_datetime(end_date)
            ...
            if provider_name == "polygon":
                all_data = await self._fetch_polygon_data(
                    symbols, start_dt, end_dt, timeframe
                )
            elif provider_name == "alpaca":
                all_data = await self._fetch_alpaca_data(
                    symbols, start_dt, end_dt, timeframe
                )
            elif provider_name == "yahoo":
                all_data = await self._fetch_yahoo_data(
                    symbols, start_dt, end_dt, timeframe
                )
            ...
            if (
                "datetime" in combined_data.columns
                and "symbol" in combined_data.columns
            ):
                combined_data = combined_data.sort(["datetime", "symbol"])
            elif (
                "timestamp" in combined_data.columns
                and "symbol" in combined_data.columns
            ):
    ...
    async def fetch_data(
        self, symbols: List[str], timeframe: str, limit: int, data_provider: str
    ) -> pl.DataFrame:
        """
        ...
        logger.info(
            f"Fetching latest {limit} data points for {symbols} with timeframe {timeframe} using {data_provider}"
        )
        ...
        try:
            ...
            if "symbol" in historical_data.columns:
                final_data = historical_data.group_by(
            ...
            logger.info(
                f"Final data returned by fetch_data for {symbols}: {final_data.to_dicts()}"
            )
            return final_data
