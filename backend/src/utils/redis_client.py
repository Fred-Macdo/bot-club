...
class RedisClient:
    def __init__(self):
        ...
        self.use_mock = (
            not REDIS_AVAILABLE
            or os.getenv("USE_MOCK_REDIS", "false").lower() == "true"
        )
    ...
    async def set_backtest_status(
        self, backtest_id: str, status: str, progress: int = 0, **kwargs
    ):
    ...
    async def delete_backtest_data(self, backtest_id: str):
        ...
        keys = [f"backtest:{backtest_id}", f"backtest:{backtest_id}:logs"]
    ...
    async def check_rate_limit(
        self, user_id: str, endpoint: str, limit: int = 60, window: int = 60
    ) -> bool:
