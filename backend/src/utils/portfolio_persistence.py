...
class PortfolioPersistence:
    ...
    def __init__(
        self, db, stream_publisher, strategy_id: str, user_id: str, mode: str = "paper"
    ):
        """
        Args:
    ...
    def save_buy(self, lot: Any) -> Optional[str]:
        ...
        try:
            lot_data = (
                lot.model_dump() if hasattr(lot, "model_dump") else lot.__dict__.copy()
            )
            ...
            if self.stream_publisher:
                self.stream_publisher(
                    "position_opened",
                    {
                        ...
                        "lot": {
                            "lot_id": lot_data.get(
                                "lot_id", str(lot_data.get("_id", ""))
                            ),
                            "symbol": lot_data.get("symbol"),
                            "quantity": float(lot_data.get("quantity", 0)),
                            "entry_price": float(lot_data.get("entry_price", 0)),
                            "entry_time": lot_data.get("entry_time").isoformat()
                            if isinstance(lot_data.get("entry_time"), datetime)
                            else str(lot_data.get("entry_time")),
                    ...
                    "trade_completed",
                    {
                        ...
                        "trade": {
                            "trade_id": trade_data.get(
                                "trade_id", str(trade_data.get("_id", ""))
                            ),
                            ...
                            "exit_price": float(trade_data.get("exit_price", 0)),
                            "entry_time": trade_data.get("entry_time").isoformat()
                            if isinstance(trade_data.get("entry_time"), datetime)
                            else str(trade_data.get("entry_time")),
                            "exit_time": trade_data.get("exit_time").isoformat()
                            if isinstance(trade_data.get("exit_time"), datetime)
                            else str(trade_data.get("exit_time")),
    ...
    def save_portfolio_snapshot(
        self, portfolio: Any, latest_prices: Dict[str, float]
    ) -> Optional[str]:
        """
        ...
        try:
                        ...
                        lots_list.append(
                            {
                                ...
                                "entry_time": lot.get("entry_time").isoformat()
                                if isinstance(lot.get("entry_time"), datetime)
                                else str(lot.get("entry_time")),
            ...
            snapshot = {
                ...
                "realized_pnl": float(
                    portfolio_data.get("performance", {}).get("total_pnl", 0)
                ),
            ...
            if self.db is not None:
                ...
                logger.info(
                    f"Saved portfolio snapshot to MongoDB: {result.inserted_id}"
                )
    ...
    def save_equity_point(
        self, total_value: Decimal, cash: Decimal, positions_value: Decimal
    ) -> Optional[str]:
        """
    ...
    def sync_portfolio_to_db(self, portfolio: Any) -> bool:
        ...
        try:
            ...
            if self.db is not None:
                ...
                logger.info(
                    f"Synced portfolio state to MongoDB for strategy {self.strategy_id}"
                )
                return True
