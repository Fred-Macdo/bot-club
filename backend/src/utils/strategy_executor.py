...
class StrategyExecutor:
    ...
    async def execute_strategy(self) -> StrategyPortfolio:
        ...
        logger.info(
            f"DCA enabled: {dca_enabled}, Max positions per symbol: {max_positions}"
        )
        ...
        for symbol in symbols:
            ...
            for row_dict in symbol_df.iter_rows(named=True):
                ...
                if current_lots:
                    ...
                    should_exit, exit_details, _ = (
                        self.condition_checker.check_exit_conditions(
                            conditions=exit_conditions, row=row_dict
                        )
                    )
                    ...
                    if should_exit:
                        ...
                        if exit_details:
                            fired = [
                                exit_conditions[i].get("indicator", "unknown")
                ...
                if can_add_position:
                    ...
                    if should_enter:
                        ...
                        allocation_pct = risk_mgmt.get(
                            "position_size_pct", risk_mgmt.get("risk_per_trade", 0.1)
                        )
                        if allocation_pct > 1:
                            allocation_pct /= 100
                        ...
                        amount_to_invest = (
                            self.portfolio.current_cash * allocation_pct_dec
                        )
    ...
    def _calculate_indicators(self) -> Dict[str, pl.DataFrame]:
        ...
        for symbol in symbols:
            ...
            if symbol_df.height > 0:
                indicator_params = convert_indicators_to_params(
    ...
    async def _get_data(self) -> pl.DataFrame:
        ...
        if self.data is None or self.data.height == 0:
            data_manager = DataManager(
                keys=self.encrypted_keys, provider_name=self.data_provider
            )
            config = self.strategy.get("config")
            if not config:
                config = self.strategy
