...
class ConditionChecker:
    ...
    def check_entry_conditions(
        self, conditions: List[Dict], row: Dict[str, Any]
    ...
    def check_exit_conditions(
        self, conditions: List[Dict], row: Dict[str, Any]
    ) -> Tuple[bool, Optional[List[bool]], Optional[Dict]]:
        """
    ...
    def _check_condition(self, row: Dict[str, Any], condition_config: Dict) -> bool:
        ...
        comparison = condition_config.get(
            "comparison", condition_config.get("operator", "")
        )
    ...
    def _resolve_current_and_prev(
        self, row: Dict[str, Any], value: Union[str, int, float]
        ...
        if isinstance(value, str):
            ...
            if value_lower in row:
                ...
                try:
                    return float(current) if current is not None else None, float(
                        prev
    ...
    def _check_crosses_above(
        self, row: Dict[str, Any], indicator_key: str, value: Union[str, int, float]
    ) -> bool:
        """
        ...
        try:
            return (
                float(prev_ind) <= prev_target and float(current_ind) > current_target
            )
        except (ValueError, TypeError):
            return False

    def _check_crosses_below(
        self, row: Dict[str, Any], indicator_key: str, value: Union[str, int, float]
    ) -> bool:
        """
