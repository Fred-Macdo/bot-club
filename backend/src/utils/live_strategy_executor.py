...
class LiveStrategyExecutor:
    ...
    async def start(self):
        ...
        config = (
            self.strategy.get("strategy_config")
            or self.strategy.get("yaml_config")
            or self.strategy.get("config")
        )
        ...
        while not self._stop_event.is_set():
            try:
                ...
                market_data = await data_manager.fetch_data(
                    symbols, timeframe, limit=25, data_provider=self.data_provider
                )

                self.logger.info(f"Market data fetched: {market_data}")
                if market_data.is_empty():
                    self.logger.warning(
                        "No market data fetched. Waiting for next interval."
                    )
    ...
    async def _process_market_data(
        self, data: pl.DataFrame, config: Dict, symbols: List[str]
    ):
        ...
        for symbol in symbols:
            ...
            if (
                symbol_df_with_indicators is None
                or symbol_df_with_indicators.is_empty()
            ):
                continue
            ...
            await self._check_exit_conditions(
                symbol, latest_data, config.get("exit_conditions", [])
            )
    ...
    async def _check_exit_conditions(
        self, symbol: str, latest_data: Dict, exit_conditions: List
    ):
        ...
        if should_exit:
            ...
            entry_time = (
                first_entry_signal["timestamp"]
                if first_entry_signal
                else latest_data["datetime"]
            )
    ...
    async def _check_entry_conditions(
        ...
        if position:
            ...
            logger.info(
                f"LIVE STRATEGY EXECUTOR: Current entry count for {symbol}: {current_entry_count}"
            )
            logger.info(f"LIVE STRATEGY EXECUTOR: Max positions : {max_positions}")
            if current_entry_count >= max_positions:
                self.logger.info(
                    f"DCA max positions ({max_positions}) reached for {symbol}. No new entry."
                )
                return

        should_enter, reason, _ = self.condition_checker.check_entry_conditions(
            conditions=entry_conditions, row=latest_data
        )
        if should_enter:
            self.logger.info(f"Entry signal for {symbol}: {reason}")

            position_sizing_method = risk_management.get(
            ...
            if position_sizing_method == "risk_based" and risk_per_trade and stop_loss:
                ...
                self.logger.warning(
                    "Position sizing method not supported or configured properly. Cannot place trade."
                )
                return

            if quantity <= 0:
                self.logger.info(
                    f"Calculated quantity for {symbol} is {quantity}. Skipping trade."
                )
                return

            stop_loss_price = (
                current_price * (1 - Decimal(str(stop_loss))) if stop_loss else None
            )
            take_profit_price = (
                current_price * (1 + Decimal(str(take_profit))) if take_profit else None
            )
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to submit order for {symbol}: {e}", exc_info=True
                )
