...
class TradingSession(BaseModel):
    ...
    strategy_name: str = Field(default="Unknown Strategy", description="Human-readable strategy name")
    ...
    # Status
    status: str = Field(
        default=TradingSessionStatus.PENDING,
    ...
    portfolio_id: Optional[str] = Field(
        None,
        description="Portfolio ID for resumption (references strategy_portfolios)"
    )
    ...
    started_at: Optional[datetime] = Field(None, description="When trading actually began")
    stopped_at: Optional[datetime] = Field(None, description="When trading stopped")
    last_iteration_at: Optional[datetime] = Field(None, description="Last successful iteration")
    updated_at: datetime = Field(
        default_factory=lambda: datetime.now(tz=timezone.utc)
    )

    # Iteration counter
    iteration_count: int = Field(default=0, description="Number of completed iterations")
