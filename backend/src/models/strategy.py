...
class Condition(BaseModel):
    ...
    comparison: str = Field(
        ..., description="Comparison operator (above, below, crosses_above, etc.)"
    )
...
class DollarCostAverage(BaseModel):
    ...
    amount_per_attempt: float = Field(
        default=100.0, description="Amount to invest per DCA attempt"
    )
...
class RiskManagement(BaseModel):
    ...
    position_sizing_method: str = Field(
        default="risk_based", description="Position sizing method"
    )
    risk_per_trade: float = Field(
        default=0.02, description="Risk per trade as decimal (2% = 0.02)"
    )
    stop_loss: float = Field(
        default=0.05, description="Stop loss as decimal (5% = 0.05)"
    )
    take_profit: float = Field(
        default=0.10, description="Take profit as decimal (10% = 0.10)"
    )
    max_position_size: float = Field(
        default=10000.0, description="Maximum position size in dollars"
    )
    atr_multiplier: float = Field(
        default=2.0, description="ATR multiplier for position sizing"
    )
...
class StrategyConfig(BaseModel):
    ...
    exit_conditions: List[Dict[str, Any]] = Field(
        default_factory=list, description="Exit conditions"
    )
    risk_management: RiskManagement = Field(
        default_factory=RiskManagement, description="Risk management settings"
    )
    indicators: List[Dict[str, Any]] = Field(
        default_factory=list, description="Required technical indicators"
    )
    ...
    def validate_symbols_uppercase(cls, v):
        ...
        if isinstance(v, list):
            upper = [
                symbol.upper() if isinstance(symbol, str) else symbol for symbol in v
            ]
