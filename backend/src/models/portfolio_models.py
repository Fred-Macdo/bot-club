...
class PositionLot(BaseModel):
    ...
    lot_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="Unique lot identifier"
    )
    ...
    cost_basis: Decimal = Field(
        default=Decimal("0"), description="Total cost (quantity * entry_price)"
    )
    ...
    alpaca_order_id: Optional[str] = Field(
        None, description="Alpaca order ID for tracking"
    )

    # Risk management — per-lot SL/TP prices calculated at entry
    stop_loss_price: Optional[Decimal] = Field(
        None, description="Stop loss price for this lot"
    )
    take_profit_price: Optional[Decimal] = Field(
        None, description="Take profit price for this lot"
    )
    ...
    def get_holding_period_days(self, current_time: datetime = None) -> int:
        ...
        entry = (
            self.entry_time
            if self.entry_time.tzinfo
            else self.entry_time.replace(tzinfo=timezone.utc)
        )
        ct = (
            current_time
...
class CompletedTrade(BaseModel):
    ...
    trade_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="Unique trade identifier"
    )
    ...
    lot_selection_method: Optional[str] = Field(
        None, description="Method used to select this lot"
    )
    ...
    def model_post_init(self, __context):
        ...
        if self.holding_period_days is None:
            ...
            exit_ = (
                self.exit_time
                if self.exit_time.tzinfo
                else self.exit_time.replace(tzinfo=timezone.utc)
            )
...
class OrderExecution(BaseModel):
    ...
    limit_price: Optional[Decimal] = Field(
        None, description="Limit price if applicable"
    )
    ...
    take_profit_order_id: Optional[str] = Field(
        None, description="Take profit order ID"
    )
...
class PortfolioSnapshot(BaseModel):
    ...
    realized_pnl: Decimal = Field(
        default=Decimal("0"), description="Cumulative realized P&L"
    )
...
class PerformanceMetrics(BaseModel):
    ...
    model_config = ConfigDict(
        json_encoders={Decimal: str}, arbitrary_types_allowed=True
    )
    ...
    largest_win: Decimal = Field(
        default=Decimal("0"), description="Largest winning trade"
    )
    largest_loss: Decimal = Field(
        default=Decimal("0"), description="Largest losing trade"
    )
    ...
    max_drawdown_duration_days: Optional[int] = Field(
        None, description="Longest drawdown period in days"
    )

    # Position metrics
    avg_holding_period_days: Optional[float] = Field(
        None, description="Average holding period in days"
    )
    max_concurrent_positions: int = Field(
        default=0, description="Maximum concurrent positions held"
    )

    # DCA metrics
    total_dca_entries: int = Field(default=0, description="Total number of DCA entries")
    avg_lots_per_position: float = Field(
        default=0.0, description="Average lots per position"
    )
    ...
    @field_validator(
        "total_pnl", "avg_win", "avg_loss", "largest_win", "largest_loss", mode="before"
    )
    @classmethod
    def parse_decimal(cls, v):
        if isinstance(v, (int, float, str)):
...
class StrategyPortfolio(BaseModel):
    ...
    model_config = ConfigDict(
        json_encoders={Decimal: str, datetime: lambda v: v.isoformat(), ObjectId: str},
        arbitrary_types_allowed=True,
    )
    ...
    lots: Dict[str, List[PositionLot]] = Field(
        default_factory=dict, description="Active lots grouped by symbol"
    )
    ...
    pending_orders: List[OrderExecution] = Field(
        default_factory=list, description="Orders awaiting execution"
    )
    ...
    equity_curve: List[PortfolioSnapshot] = Field(
        default_factory=list, description="Historical portfolio snapshots"
    )

    # Settings
    lot_selection_method: LotSelectionMethod = Field(
        default=LotSelectionMethod.FIFO, description="Default lot selection method"
    )
    ...
    def set_initial_capital(self, amount: Decimal):
        """Set initial capital and current cash"""
        self.current_cash = amount

    def add_buy(self, lot: PositionLot):
        """Add a buy lot to the portfolio"""
    ...
    def process_sell(
        ...
        """
        Process a sell order, matching against existing lots based on selection method.
        Handles partial fills across multiple lots (FIFO/LIFO/etc).
        """
        ...
        # Get lots and sort based on selection method
        lots = self.lots[symbol]

        if self.lot_selection_method == LotSelectionMethod.LIFO:
            lots.sort(key=lambda x: x.entry_time, reverse=True)
        elif self.lot_selection_method == LotSelectionMethod.HIFO:
            lots.sort(key=lambda x: x.entry_price, reverse=True)
        elif self.lot_selection_method == LotSelectionMethod.LOFO:
            lots.sort(key=lambda x: x.entry_price)
        else:  # FIFO
            lots.sort(key=lambda x: x.entry_time)
        ...
        for lot in lots:
            ...
            if lot.quantity <= remaining_qty:
                ...
                trade = CompletedTrade(
                    ...
                    exit_time=exit_time,
                    exit_reason=reason,
                    realized_pnl=(exit_price - lot.entry_price) * lot.quantity,
                    strategy_id=lot.strategy_id,
                    user_id=lot.user_id,
                    lot_selection_method=self.lot_selection_method.value,
                )
                ...
                trade = CompletedTrade(
                    ...
                    exit_time=exit_time,
                    exit_reason=reason,
                    realized_pnl=(exit_price - lot.entry_price) * trade_qty,
                    strategy_id=lot.strategy_id,
                    user_id=lot.user_id,
                    lot_selection_method=self.lot_selection_method.value,
                )
                ...
                # Update lot
                lot.quantity -= trade_qty
                lot.cost_basis = lot.quantity * lot.entry_price
        ...
        if remaining_qty > 0:
            raise ValueError(
                f"Insufficient quantity to sell. Request: {quantity}, Available: {quantity - remaining_qty}"
            )
        ...
        self.completed_trades.extend(processed_trades)
        self.current_cash += quantity * exit_price

        return processed_trades

    def update_performance_metrics(self):
        """
        Recalculate performance metrics based on trade history
        """
        ...
        total_holding_days = sum(
            t.holding_period_days for t in trades if t.holding_period_days is not None
        )
        avg_holding_period = (
            total_holding_days / total_trades if total_trades > 0 else 0.0
        )
        ...
        if self.initial_capital > 0:
            self.performance.total_pnl_pct = float(
                total_pnl / self.initial_capital * 100
            )

        self.performance.avg_win = avg_win
        self.performance.avg_loss = avg_loss
    ...
    def update_equity_curve(self, current_prices: Dict[str, Decimal]):
        ...
        realized_pnl = sum(t.realized_pnl for t in self.completed_trades)

        # Count items
        position_count = len(self.lots)
        lot_count = sum(len(lots) for lots in self.lots.values())
        ...
        # Update max drawdown if we have enough history
        if self.equity_curve:
            ...
            self.performance.max_drawdown = float(max_dd * 100)
...
class StrategyPortfolioDocument(BaseModel):
    ...
    lots: Dict[str, List[Dict[str, Any]]] = Field(
        default_factory=dict, description="Serialized lots"
    )
    completed_trades: List[Dict[str, Any]] = Field(
        default_factory=list, description="Serialized trades"
    )
    pending_orders: List[Dict[str, Any]] = Field(
        default_factory=list, description="Serialized orders"
    )
