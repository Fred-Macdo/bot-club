...
class StrategyPortfolio(BaseModel):
    ...
    def process_sell(self, symbol: str, quantity: Decimal, exit_price: Decimal, exit_time: datetime, reason: Optional[str] = None) -> List[CompletedTrade]:
        """
        ...
        if remaining_qty > 0:
             raise ValueError(f"Insufficient quantity to sell. Request: {quantity}, Available: {quantity - remaining_qty}")
    ...
    def update_performance_metrics(self):
        ...
        total_holding_days = sum(t.holding_period_days for t in trades if t.holding_period_days is not None)
        avg_holding_period = total_holding_days / total_trades if total_trades > 0 else 0.0
        ...
        if self.initial_capital > 0:
            self.performance.total_pnl_pct = float(total_pnl / self.initial_capital * 100)
