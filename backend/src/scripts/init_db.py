...
from services.default_strategies import get_default_strategies  # noqa: E402, F401
from models.strategy import Strategy  # noqa: E402, F401
from utils.mongo_helpers import PyObjectId  # noqa: E402
...
async def initialize_default_strategies_collection():
    ...
    try:
        ...
        for filename in strategy_files:
            ...
            if yaml_path.exists():
                try:
                    ...
                    result = await db.default_strategies.insert_one(
                        default_strategy_doc
                    )
                    logger.info(
                        f"Loaded default strategy: {strategy_data['name']} with ID: {result.inserted_id}"
                    )
                    loaded_count += 1
        ...
        logger.info(
            f"Successfully loaded {loaded_count} default strategies into collection"
        )
        return loaded_count
        ...
        for collection_name in collections_after:
            ...
            if count > 0:
                ...
                if collection_name == "default_strategies":
                    logger.info(
                        f"Sample from '{collection_name}': {sample['template_name']} - {sample['template_description']}"
                    )
                else:
...
async def create_user_strategies_from_defaults(db, user_id: PyObjectId):
    ...
    try:
        ...
        async for default_strategy in default_strategies_cursor:
            ...
            logger.info(
                f"Created user strategy: {yaml_config['name']} with ID: {result.inserted_id}"
            )
            created_count += 1
