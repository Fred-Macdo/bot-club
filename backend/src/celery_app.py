...
celery_app = Celery(
    ...
    broker=REDIS_URL,
    backend=REDIS_URL,
    include=["src.tasks.trading_tasks", "src.tasks.backtest_task"],
)
...
celery_app.conf.task_acks_late = (
    True  # Acknowledge tasks after execution (safer on worker crash)
)
celery_app.conf.worker_prefetch_multiplier = (
    1  # Prevent greedy prefetch for long-running tasks
)
