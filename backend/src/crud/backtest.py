...
async def create_backtest(
    db: Database, user_id: Union[str, PyObjectId], backtest_data: dict
) -> Backtest:
    ...
    try:
        ...
        result = await run_db_operation(
            backtest_collection.insert_one, backtest.dict(by_alias=True)
        )

        # Retrieve the created backtest
        created_backtest = await run_db_operation(
            backtest_collection.find_one, {"_id": result.inserted_id}
        )
...
async def get_backtest_by_id(
    db: Database, backtest_id: Union[str, PyObjectId], user_id: Union[str, PyObjectId]
) -> Optional[Backtest]:
    ...
    try:
        backtest_doc = await run_db_operation(
            backtest_collection.find_one, {"_id": backtest_id, "user_id": user_id}
        )
...
async def get_backtests_by_user_id(
    db: Database, user_id: Union[str, PyObjectId], limit: int = 50, skip: int = 0
    ...
    try:
        ...
        backtests_raw = await run_db_operation(
            execute_aggregate, backtest_collection, pipeline
        )
...
async def get_backtests_by_strategy_id(
    ...
    try:
        pipeline = [
            {"$match": {"strategy_id": strategy_id, "user_id": user_id}},
            {
                "$lookup": {
                    "from": STRATEGY_COLLECTION,
                    "localField": "strategy_id",
...
async def update_backtest(
    ...
    try:
        ...
        if result.modified_count > 0:
            updated_backtest = await run_db_operation(
                backtest_collection.find_one, {"_id": backtest_id}
            )
...
async def delete_backtest(
    ...
    try:
        result = await run_db_operation(
            backtest_collection.delete_one, {"_id": backtest_id, "user_id": user_id}
        )
...
async def get_backtest_count_by_user(
    db: Database, user_id: Union[str, PyObjectId]
) -> int:
    ...
    try:
        count = await run_db_operation(
            backtest_collection.count_documents, {"user_id": user_id}
        )
        return count
    except Exception as e:
...
async def create_backtest_execution(db: Database, execution: BacktestExecution) -> str:
...
async def update_backtest_execution(
    db: Database, backtest_id: str, update_data: dict
) -> bool:
    ...
    result = await run_db_operation(
        collection.update_one, {"_id": ObjectId(backtest_id)}, {"$set": update_data}
    )
...
async def get_backtest_execution(
    db: Database, backtest_id: str, user_id: str
) -> Optional[BacktestExecution]:
    """Get backtest execution by ID"""
    collection = db["backtest_executions"]

    execution_data = await run_db_operation(
        collection.find_one,
        {"_id": ObjectId(backtest_id), "user_id": ObjectId(user_id)},
    )
...
async def get_backtest_results_from_db(db: Database, backtest_id: str):
    ...
    return await run_db_operation(
        db.backtest_executions.find_one, {"backtest_id": backtest_id}
    )


async def get_strategy_for_backtest(
    db: Database, strategy_id_str: str, user_id: ObjectId
    ...
    user_strategy = await run_db_operation(
        db.strategy.find_one,
        {
            "_id": strategy_obj_id,
    ...
    user_strategy_plural = await run_db_operation(
    ...
    if user_strategy_plural:
        print(
            f"[DEBUG] Found in 'strategies' collection: {user_strategy_plural['name']}"
        )
        return user_strategy_plural

    # Try default strategies collection
    default_strategy = await run_db_operation(
        db.default_strategies.find_one, {"_id": strategy_obj_id}
    )

    if default_strategy:
        print(
            f"[DEBUG] Found in 'default_strategies' collection: {default_strategy['name']}"
        )
