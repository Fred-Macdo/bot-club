...
async def get_strategies_by_user_id(
    db: AsyncIOMotorDatabase, user_id: Union[str, PyObjectId]
) -> List[Strategy]:
    ...
    query = {"$or": [{"user_id": user_id}, {"user_id": user_id_str}]}

    # Count total documents
    total_count = await run_db_operation(strategies_collection.count_documents, query)
    print(f"DEBUG CRUD: Found {total_count} strategies for user_id: {user_id}")
    strategies = []
...
async def get_strategy_by_id(
    db: AsyncIOMotorDatabase, strategy_id: PyObjectId, user_id: PyObjectId
) -> Optional[Strategy]:
    """Get a strategy by ID (ensuring it belongs to the user)"""
    strategy_data = await run_db_operation(
        db[STRATEGY_COLLECTION].find_one, {"_id": strategy_id, "user_id": user_id}
    )
    if strategy_data:
...
async def create_strategy(
    db: AsyncIOMotorDatabase, strategy_data: UserStrategy, user_id: PyObjectId
) -> Strategy:
...
async def update_strategy(
    ...
    update_dict = update_data.model_dump(
        exclude_unset=True, exclude={"id", "user_id", "created_at", "updated_at"}
    )
...
async def delete_strategy(
    ...
    """Delete a strategy"""
    result = await run_db_operation(
        db[STRATEGY_COLLECTION].delete_one, {"_id": strategy_id, "user_id": user_id}
    )
...
async def save_backtest_result(
    db: AsyncIOMotorDatabase, strategy_id: PyObjectId, backtest_result: Backtest
) -> Backtest:
...
async def get_backtest_results_by_strategy(
    db: AsyncIOMotorDatabase, strategy_id: PyObjectId
) -> List[Backtest]:
    """Get all backtest results for a strategy"""
    results = []
    # Use execute_find helper with sort
    raw_results = await run_db_operation(
        execute_find,
        db[BACKTEST_COLLECTION],
...
async def get_backtest_result_by_id(
    db: AsyncIOMotorDatabase, backtest_id: PyObjectId
) -> Optional[Backtest]:
    """Get a specific backtest result by ID"""
    result_data = await run_db_operation(
        db[BACKTEST_COLLECTION].find_one, {"_id": backtest_id}
    )
    if result_data:
...
async def delete_backtest_results_by_strategy(
    ...
    result = await run_db_operation(
        db[BACKTEST_COLLECTION].delete_many, {"strategy_id": strategy_id}
    )
    return result.deleted_count > 0
...
async def get_strategy_by_id_cached(
    db: AsyncIOMotorDatabase, strategy_id: str, user_id: str
) -> Optional[Strategy]:
