...
async def get_user_config(
    ...
    try:
        ...
        config_doc = await run_db_operation(
            db.user_config.find_one, {"user_id": str(current_user.id)}
        )
        ...
        if config_doc.get("alpaca_paper_secret_key"):
            config_doc["alpaca_paper_secret_key"] = ConfigEncryption.decrypt_value(
                config_doc["alpaca_paper_secret_key"]
            )
        if config_doc.get("alpaca_live_secret_key"):
            config_doc["alpaca_live_secret_key"] = ConfigEncryption.decrypt_value(
                config_doc["alpaca_live_secret_key"]
            )
        if config_doc.get("polygon_secret_key"):
            config_doc["polygon_secret_key"] = ConfigEncryption.decrypt_value(
                config_doc["polygon_secret_key"]
            )
    ...
    try:
        ...
        if "alpaca_paper_api_key" in config_data:
            ...
            if config_data.get("alpaca_paper_endpoint"):
                update_data["alpaca_paper_endpoint"] = config_data[
        ...
        if "alpaca_live_api_key" in config_data:
            ...
            if config_data.get("alpaca_live_endpoint"):
                update_data["alpaca_live_endpoint"] = config_data[
...
async def save_polygon_config(
    ...
    try:
        ...
        if config_data.get("polygon_secret_key"):
            update_data["polygon_secret_key"] = ConfigEncryption.encrypt_value(
                config_data["polygon_secret_key"]
            )
...
async def delete_polygon_config(
    ...
    try:
        ...
        await run_db_operation(
            ...
            {"user_id": str(current_user.id)},
            {
                "$unset": {"polygon_api_key_name": "", "polygon_secret_key": ""},
                "$set": {"updated_at": datetime.now(timezone.utc)},
            },
        )
