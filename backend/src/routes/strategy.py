...
async def get_user_strategies(
    ...
    try:
        ...
        print(
            f"DEBUG: Found {len(strategies)} strategies in database for user {current_user.id}"
        )
        for strategy in strategies:
            print(
                f"DEBUG: Strategy - ID: {strategy.id}, Name: {strategy.name}, User ID: {strategy.user_id}"
            )
...
async def update_existing_strategy(
    ...
    updated_strategy = await update_strategy(
        db, strategy_obj_id, strategy_update, current_user.id
    )
...
async def toggle_strategy_trading(
    ...
    updated_strategy = await toggle_strategy_status(
        db, strategy_obj_id, current_user.id, is_active
    )
    if not updated_strategy:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Strategy not found"
