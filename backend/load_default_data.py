...
def parse_yaml_to_strategy(yaml_data: dict) -> StrategyCreate:
    ...
    for indicator_data in yaml_data.get("indicators", []):
        indicators.append(
            Indicator(
                name=indicator_data["name"], params=indicator_data.get("params", {})
            )
        )
    ...
    risk_management = RiskManagement(
        position_sizing_method=risk_mgmt_data.get(
    ...
    try:
        ...
        for filename in yaml_files:
            ...
            try:
                ...
                print(
                    f"✓ Loaded strategy: '{strategy.name}' with ID: {result.inserted_id}"
                )
                loaded_count += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Load default strategy YAML files into MongoDB"
    )
    parser.add_argument(
        "--yaml-dir",
        type=str,
    ...
    try:
        loaded_count = asyncio.run(
            load_default_data(args.yaml_dir, args.db_uri, args.db_name)
        )
