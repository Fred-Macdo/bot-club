...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const logMessages = [
        'Checking entry conditions for AAPL',
        'Market data updated',
        'Position size calculated',
        'Order validation complete',
        'Signal generated for TSLA'
      ];
  ...
  const handleDeployStrategy = async () => {
    ...
    try {
      ...
      setLogs(prev => [{
        timestamp: new Date(),
        level: 'INFO',
        message: `Strategy deployed: ${selectedStrategy?.name}`
      }, ...prev]);
  ...
  const handleStopStrategy = async () => {
    ...
    try {
      ...
      setLogs(prev => [{
        timestamp: new Date(),
        level: 'INFO',
        message: `Strategy stopped: ${selectedStrategy?.name}`
      }, ...prev]);
    } catch (error) {
  ...
  const mockTrades = [
    { id: 1, symbol: 'AAPL', side: 'BUY', quantity: 100, entryPrice: 150.25, exitPrice: null, entryTime: new Date(Date.now() - 3600000), exitTime: null, pnl: -125.50, status: 'OPEN' },
    { id: 2, symbol: 'TSLA', side: 'SELL', quantity: 50, entryPrice: 245.80, exitPrice: 240.15, entryTime: new Date(Date.now() - 7200000), exitTime: new Date(Date.now() - 1800000), pnl: 282.50, status: 'CLOSED' }
  ];
